import { Coins, ShoppingCart, Swords } from 'lucide-react';
import { memo } from 'react';
import type { Phase, TurnState } from '../types';
...
const TurnInfo = memo(function TurnInfo({
  turnState,
  phase,
  turnNumber,
  currentPlayer,
  ...
  return (
    <div className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 z-40 flex flex-col items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-white/70 dark:bg-slate-800/70 backdrop-blur-sm rounded-lg border border-slate-300 dark:border-slate-600 shadow-xl">
      ...
      <div className="flex flex-col items-center gap-0.5 sm:gap-1 pb-1.5 sm:pb-2 border-b border-slate-300 dark:border-slate-600 w-full">
        ...
        <span
          className={`text-[9px] sm:text-[10px] px-1.5 sm:px-2 py-0.5 rounded-full font-medium ${phaseColor(phase)}`}
        >
      ...
      <div className="flex flex-col items-center gap-0.5 sm:gap-1">
        ...
        <span className="text-[9px] sm:text-[10px] text-slate-600 dark:text-slate-400 font-medium">
          アクション
        </span>
      ...
      <div className="flex flex-col items-center gap-0.5 sm:gap-1">
        ...
        <span className="text-lg sm:text-xl font-black text-yellow-600 dark:text-yellow-400">
          {turnState.coins}
        </span>
      </div>
    </div>
